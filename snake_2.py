...
SPEED = 0.36 # velocidade da cobra
SNAKE_SIZE = 9 # tamanho que a cobrinha inicia o jogo
APPLE_SIZE = SNAKE_SIZE # tamanho da comida que a cobrinha come
SEPARATION = 10
SCREEN_HEIGHT = 600 # altura da janela
SCREEN_WIDTH = 800 # largura da janela
FPS = 25 # quantidade de frames por segundo
KEY = {"UP":1,"DOWN":2,"LEFT":3,"RIGHT":4} # teclas que são selecionadas
...
def checkCollision(posA,As,posB,Bs):
    #As é o tamanho de a | Bs é o tamanho de B
    if(posA.x   < posB.x+Bs and posA.x+As > posB.x and posA.y < posB.y + Bs and posA.y+As > posB.y):
        return True
    return False

# verificando os limites da tela
def checkLimits(entity): # entity é a cobra que vc está criando
...
# Definindo a classe que cria a comida da cobra
class Apple:
    def __init__(self,x,y,state):
        self.x = x # eixo x
        self.y = y # eixo y
        self.state = state # estado  - bloeano
        self.color = pygame.color.Color("red") # cor da maçã

    #desenhando a maçã na tela
    def draw(self,screen):
        pygame.draw.rect(screen,self.color,(self.x,self.y,APPLE_SIZE,APPLE_SIZE),0)

# Definindo a classe que cria as partes da cobra
class Segment:
...
# Definindo class que cria a cobra
class Snake:
    def __init__(self,x,y):
        self.x = x
        self.y = y
        self.direction = KEY["UP"]
        self.stack = [] # cirando pilha que compõe o tamanho da cobra 
